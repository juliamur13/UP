(function() {
    ...
    function editPhotoPost(id, photoPost) {
        var index = photoPosts.findIndex(function(obj) {
            return obj.id === id
        });
        if (index !== -1) {
            if (photoPost.description !== undefined && photoPost.description.length !== 0) {
                document.getElementsByClassName("description_text")[photoPosts.length - index - 1].textContent = photoPost.description;
            ...
            if (photoPost.hashTags !== undefined && photoPost.hashTags.length !== 0) {
                document.getElementsByClassName("hashTag_text")[photoPosts.length - index - 1].textContent = photoPost.hashTags;
                photoPosts[photoPosts.length - index - 1].hashTags = photoPost.hashTags;
            }
            if (photoPost.likes !== undefined && photoPost.likes.length !== 0) {
                document.getElementsByClassName("likes_text")[photoPosts.length - index - 1].textContent = photoPost.likes;
                photoPosts[photoPosts.length - index - 1].likes = photoPost.likes;
            }
            if (photoPost.photoLink !== undefined && photoPost.photoLink.length !== 0) {
                document.getElementsByClassName("photo")[photoPosts.length - index - 1].src = photoPost.photoLink;
    ...
    function addPhotoPost(photoPost) {
        ...
        comment.innerHTML = '<p class="user_name_comments">' + '<img class="user_comments_photo" src="site/user.png"/>Имя пользователя</p> ' + '<p class ="comment">Комментарий</p>';

        newElem.innerHTML = '<div class="user_panel">' + '\<div>' + '\<p class="user_name_text_post"> ' + '<img class="user_name_photo_post" src="site/user.png" />' + photoPost.author + '</p>' + '</div>' + '<div class="buttons">' + '\<a class="link_edit_delete" ><img class="button_edit" src="site/delete.png" /></a>' + '<a class="link_edit_like"  ><img class="button_edit_like" src="site/like4.png" /></a>' + '<a class="link_edit"  ><img class="button_edit" src="site/edit.png"/></a>' + '</div>' + '</div>' + '<div class="photopost">' + '\<div class="user_photopost">' + '<img class="photo" src="' + photoPost.photoLink + '"><p class="date_post">' + photoPost.createdAt + '</p>' + '<div class="description">' + '\<p class="description_text">' + photoPost.description + '</p><p class="hashTag_text">' + photoPost.hashTags + '</p><p class="likes_text">' + photoPost.likes + '</p>' + '</div>' +
            '</div>' + '<div class="comments">' + '\<div class="comments_overflow"> ' + '\<div id="list_comments">' + '</div>' + '</div>' + '<input type="text" class="text" placeholder="Написать коментарий..." />' + '</div>' + '</div>';
