...
var moduleDOM = (function() {
    ...
    function displayPhotoPosts(posts, user) {
        controlUser(user);
        var COUNT_ELEMENTS = 10;
        var countPostsDisplay;
        var countPostsHtml = document.getElementsByClassName('post').length;
        if (posts.length < COUNT_ELEMENTS + countPostsHtml) {
            countPostsDisplay = posts.length;
        } else {
            countPostsDisplay = COUNT_ELEMENTS + countPostsHtml;
        }
        for (var item = 0; item < countPostsHtml; item++) {
            document.getElementsByClassName('post')[item].classList.add('hide');
        }
        for (var index = posts.length - countPostsDisplay; index < posts.length; index++) {
            addPhotoPost(posts[index], user);
        }
